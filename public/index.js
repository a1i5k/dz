...
class App {
  ...
  onInputKeydown = async ({ key, target }) => {
    if (key === 'Enter') {
      if (target.value) {
        const re = /([А-Я]* [А-Я]\.[А-Я]\.)/g;
        if (!(target.value.match(re) || []).length) {
          alert("Неверный формат!\nФормат записи тестировщика:\nФамилия И.О.")
          return;
        }
...
class Tasklist {
  ...
  onAddTaskButtonClick = async () => {
    let newTaskName = prompt('Добавить название тестирования');
    const newAddressName = prompt('Добавить адрес тестирования');
    newTaskName = newTaskName + ' ' + newAddressName;
    if (!newTaskName || !newAddressName) return;
  ...
  onEditTask = async (taskID) => {
    ...
    const newTaskName = prompt('Введите новый название и адрес тестирования', oldTaskName);
  ...
  onDeleteTaskButtonClick = async (taskID) => {
    ...
    if (!confirm(`Тестирование '${taskName}' будет удалёно. Продолжить?`)) return;
